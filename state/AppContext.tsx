...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      if (apiState) {
        ...
        dispatch({
          type: 'INITIALIZE_STATE',
          payload: { ...localState, menuItems: initialMenuItems, isLoading: false },
        });
      }
    };
  ...
  const updateOrderStatus = (orderId: string, status: OrderStatus) => {
    const tx = state.transactions.find(t => t.id === orderId);
    if (tx) {
        const updatedTx = { ...tx, status } as Transaction;
        dispatch({ type: 'UPDATE_ORDER_STATUS', payload: { orderId, status } });
        api.syncTransaction(updatedTx);
    }
  };
  ...
  const markOrderPaid = (orderId: string) => {
    const tx = state.transactions.find(t => t.id === orderId);
    if (tx && tx.type === 'order') {
        const updatedTx = { ...tx, isPaid: true } as Transaction;
  ...
  const confirmLoad = (transactionId: string) => {
      ...
      if (tx && tx.type === 'load') {
          const updatedTx = { ...tx, status: 'Confirmed' } as Transaction;
  ...
  const requestWater = () => {
    ...
    const tx = { ...request, type: 'service' } as Transaction;
    dispatch({ type: 'REQUEST_SERVICE', payload: tx });
    api.syncTransaction(tx);
  ...
  const resolveServiceRequest = (id: string) => {
    ...
    if (tx && tx.type === 'service') {
        const updatedTx = { ...tx, status: 'Resolved' } as Transaction;
